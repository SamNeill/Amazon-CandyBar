...
public class HomeAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private class UpdateChecker extends AsyncTaskBase {
        ...
        protected boolean run() {
            if (!isCancelled()) {
                try {
                    // Add check for Amazon IAP availability
                    if (!isAmazonIAPAvailable()) {
                        return false;
                    }
        ...
        private boolean isAmazonIAPAvailable() {
            try {
                // Check if Amazon IAP is available
                return mContext.getPackageManager().getPackageInfo("com.amazon.venezia", 0) != null;
    ...
    private void initDonationAndPremium() {
        CandyBarApplication app = (CandyBarApplication) mContext.getApplicationContext();
        
        // Premium Request
        if (app.isPremiumRequestEnabled()) {
            String[] productIds = app.getPremiumRequestProductIds();
            int[] requestCounts = app.getPremiumRequestCounts();
            String[] prices = app.getPremiumRequestPrices();
            
            // Implementation code for premium requests...
            for (int i = 0; i < productIds.length; i++) {
                String productId = productIds[i];
                int count = requestCounts[i];
                String price = prices[i];
                // Use these values to set up premium request options
            }
        }
        
        // Donation
        if (app.isDonationEnabled()) {
            String[] productIds = app.getDonationProductsId();
            String[] prices = app.getDonationProductsPrices();
            String[] names = app.getDonationProductNames();
            
            // Implementation code for donations...
            for (int i = 0; i < productIds.length; i++) {
                String productId = productIds[i];
                String price = prices[i];
                String name = names[i];
                // Use these values to set up donation options
            }
        }
    }
}
